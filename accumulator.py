...
class AccumulatorWithVariance(dict):
    """Accumulate values and calculate their average and variance on the fly. """
    ...
    def __getitem__(self, key):
        sigma = np.sqrt(self._av2[key] - self._av[key]**2)
        str_out = "%s = %e +/- %e" % (key, self._av[key], sigma)
        return str_out
        
    def __repr__(self):
        """If acc is an AccumulatorWithVariance object, use 
              fh.write(str(acc))
           to output the statistics to the file handle fh.
        """
        s = ""
        for k in self.accumulator_keys:
